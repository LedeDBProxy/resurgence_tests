...
local MYSQL_USER	 	= os.getenv("MYSQL_USER")	 	or "test"
...
PROXY_LUAPATH  = os.getenv("PROXY_LUAPATH")  	or top_builddir .. "/lib/admin.lua"
...
function options_tostring(tbl, sep)
	...
	for k, v in pairs(tbl) do
		...
		for tk, tv in pairs(values) do
			...
			if enc_value ==  "with_no_value" then
				s = s .. "--" .. k  .. " "
			else
                if k == "plugins" and type(enc_value) == "list" then
                    for i = 1, #enc_value do
                        s = s .. "--" .. k .. "=\"" .. enc_value .. "\" "
                    end
                else
                    s = s .. "--" .. k .. "=\"" .. enc_value .. "\" "
